...
local MAJOR_VERSION = "LibFishing-1.0-KUI"
...
local GetItemCooldown = GetItemCooldown or C_Container_GetItemCooldown or (C_Container and C_Container.GetItemCooldown)
...
function FishLib:UpdateLureInventory()
	...
	for _,lure in ipairs(FISHINGLURES) do
		...
		if ( count > 0 ) then
			local startTime, _, _ = SafeGetItemCooldown(id);
			if (startTime == 0) then
...
local function UseThisLure(lure, b, enchant, skill, level)
	if ( lure ) then
		local startTime, _, _ = SafeGetItemCooldown(lure.id);
		level = level or 0;
