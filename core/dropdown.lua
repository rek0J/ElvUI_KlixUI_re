...
local DD = KUI:NewModule("Dropdown", "AceEvent-3.0")
...
local function CreateListButton(frame)
	...
	button.hoverTex:SetVertexColor(T.unpack(E["media"].rgbvaluecolor))
...
function KUI:DropDown(list, frame, MenuAnchor, FramePoint, xOffset, yOffset, parent, customWidth, justify)
		...
		for i=1, #list do
			...
			if list[i].secure then
				...
				elseif btn.secure.buttonType == "macro" then
					...
					KUI:Print("Wrong argument for button type: "..btn.secure.buttonType)
				end
			end
...
function DD:RegisterMenu(menu)
	...
	if name then
		...
		KUI:Print("Dropdown not registered. Please check if it has a name.")
	end
end
