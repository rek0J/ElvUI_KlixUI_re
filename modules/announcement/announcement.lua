...
function AS:SendAddonMessage(message)
	if C_ChatInfo and C_ChatInfo.SendAddonMessage then
		...
		-- MoP Classic: C_ChatInfo.SendAddonMessage not available, skip
	end
end
...
function AS:Initialize()
	...
	self.AllUsers = {}
	local regStatus = true
	if C_ChatInfo and C_ChatInfo.RegisterAddonMessagePrefix then
		regStatus = C_ChatInfo.RegisterAddonMessagePrefix(self.Prefix)
		if not regStatus then T.print("[KlixUI Announce System] Prefix error") end
	else
		-- MoP Classic: C_ChatInfo.RegisterAddonMessagePrefix not available, skip
	end
