...
local function GetCoordPanels()
	local xcoord = loc_panel and loc_panel.Xcoord or _G.KUI_LocPanel_X
	local ycoord = loc_panel and loc_panel.Ycoord or _G.KUI_LocPanel_Y
...
LP.Spells = {
	...
	["HUNTER"] = {},
	["MAGE"] = {
		[1] = {text = T.GetSpellInfo(193759),icon = KUI:GetIconFromID("spell", 193759),secure = {buttonType = "spell",ID = 193759}, UseTooltip = true}, --Guardian place
	},
	["MONK"] = {
	...
	["DarkIronDwarf"] = {
		...
		["Vulpera"] = {
			[1] = {text = T.GetSpellInfo(312370),icon = KUI:GetIconFromID("spell", 312370),secure = {buttonType = "spell",ID = 312370}, UseTooltip = true}, -- Make Camp
			[2] = {text = T.GetSpellInfo(312372),icon = KUI:GetIconFromID("spell", 312372),secure = {buttonType = "spell",ID = 312372}, UseTooltip = true}, -- Return to Camp
		},
	},
	["teleports"] = {
		["Horde"] = {
...
local function Bar_OnEnter(self)
	...
	GameTooltip:Point("BOTTOM", self, "BOTTOM", 0, 0)
...
function LP:CreateLocationPanel()
	...
	loc_panel.Xcoord.Text = loc_panel.Xcoord:CreateFontString(nil, "OVERLAY")
	...
	LP.Menu1:CreateBackdrop("Transparent", true)
	LP.Menu2 = T.CreateFrame("Frame", "KUI_LocPanel_RightClickMenu2", E.UIParent)
	LP.Menu2:CreateBackdrop("Transparent", true)
...
function LP:UpdateCoords(elapsed)
	...
	local xcoord, ycoord = GetCoordPanels()
	if not xcoord or not ycoord or not xcoord.Text or not ycoord.Text then
		LP.elapsed = 0
		return
	end
	...
	if T.tonumber(E.version) >= 10.78 and E.MapInfo then
		...
		xcoord.Text:SetText(x)
		ycoord.Text:SetText(y)
	...
	xcoord.Text:SetTextColor(colorC.r, colorC.g, colorC.b)
	ycoord.Text:SetTextColor(colorC.r, colorC.g, colorC.b)
...
function LP:Resize()
	...
	if xcoord then xcoord:Size(LP.db.fontSize * 3, LP.db.height) end
	if ycoord then ycoord:Size(LP.db.fontSize * 3, LP.db.height) end
end
...
function LP:Fonts()
	...
	if xcoord and xcoord.Text then xcoord.Text:SetFont(LSM:Fetch('font', LP.db.font), LP.db.fontSize, LP.db.fontOutline) end
	if ycoord and ycoord.Text then ycoord.Text:SetFont(LSM:Fetch('font', LP.db.font), LP.db.fontSize, LP.db.fontOutline) end
...
function LP:Template()
	...
	loc_panel:CreateBackdrop(LP.db.template)
	if xcoord then xcoord:CreateBackdrop(LP.db.template) end
	if ycoord then ycoord:CreateBackdrop(LP.db.template) end
end
...
function LP:Style()
	...
	if E.db.KlixUI.general.style then
		if db.style then
			...
			loc_panel:Styling()
			if xcoord then xcoord:Styling() end
			if ycoord then ycoord:Styling() end
			LP.Menu1:Styling()
			LP.Menu2:Styling()
		end
	end
...
function LP:UNIT_AURA(_, unit)
	...
	if LP.db.enable and LP.db.orderhallhide then
		local inOrderHall = C_Garrison_IsPlayerInGarrison(GarrisonType_7_0)
		if inOrderHall then
