...
function FriendsCount_OnEvent(event, ...)
	...
	_G.KUI_FriendsCounter:SetText(bnetCount.."|cff416380/200|r")
end
...
local function styleFriends()
	...
	_G.FriendsFrameIcon:SetTexture([[Interface\AddOns\ElvUI_KlixUI\media\textures\Bnet]])
	_G.FriendsFrameIcon:SetSize(36, 36)

	hooksecurefunc(_G.FriendsFrameIcon, "SetTexture", function(self, texture)
		if texture ~= [[Interface\AddOns\ElvUI_KlixUI\media\textures\Bnet]] then
			self:SetTexture([[Interface\AddOns\ElvUI_KlixUI\media\textures\Bnet]])
		end
	end)
