...
local function styleGarrison()
	...
	for i = 1, _G.GARRISON_NUM_BUILDING_SIZES do
		...
		local bg = T.CreateFrame("Frame", nil, tab, 'BackdropTemplate')
	...
	hooksecurefunc("GarrisonCapacitiveDisplayFrame_Update", function()
		...
		while reagent do
			...
			local bg = T.CreateFrame("Frame", nil, reagent, 'BackdropTemplate')
			bg:SetPoint("TOPLEFT")
	...
	for i = 1, 10 do
		T.select(i, GarrisonLandingPage:GetRegions()):Hide() -- Parchment
	end
	...
	for i = 1, #buttons do
		...
		local bg = T.CreateFrame("Frame", nil, button, 'BackdropTemplate')
		bg:SetPoint("TOPLEFT")
	...
	T.select(2, _G.GarrisonShipyardFrameFollowers:GetRegions()):Hide()
	...
	KS:ReskinTab(_G.GarrisonShipyardFrameTab1)
	KS:ReskinTab(_G.GarrisonShipyardFrameTab2)
