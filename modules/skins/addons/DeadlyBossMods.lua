...
local DBMSkin = T.CreateFrame("Frame")
DBMSkin:RegisterEvent("PLAYER_LOGIN")
DBMSkin:RegisterEvent("ADDON_LOADED")
DBMSkin:SetScript("OnEvent", function(self, event, addon)
	if E.private.KlixUI == nil then E.private.KlixUI = {} end
	if E.private.KlixUI.skins == nil then E.private.KlixUI.skins = {} end
	if E.private.KlixUI.skins.addonSkins == nil then E.private.KlixUI.skins.addonSkins = {} end
	if E.private.KlixUI.skins.addonSkins.dbm ~= true then return end
				...
				if not bar.injected then
					bar.ApplyStyle = function()
						...
						if icon1.overlay then
							...
							icon1.overlay = T.CreateFrame("Frame", "$parentIcon1Overlay", tbar)
						...
						if icon2.overlay then
							...
							icon2.overlay = T.CreateFrame("Frame", "$parentIcon2Overlay", tbar)
						...
						if not icon1.styled then
							icon1:SetTexCoord(T.unpack(E.TexCoords))
						...
						if not icon2.styled then
							icon2:SetTexCoord(T.unpack(E.TexCoords))
	...
	if T.IsAddOnLoaded("DBM-GUI") then
		T.table_insert(_G.UISpecialFrames, "DBM_GUI_OptionsFrame")
		...
		for i = 1, T.table_getn(dbmbuttons) do
		...
		for i = 1, T.table_getn(dbmtabs) do
