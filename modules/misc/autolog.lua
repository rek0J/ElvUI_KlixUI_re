...
local function GetMapInfo(mapID)
	if not mapID or not T.C_Map_GetMapInfo then return end

	local info = T.C_Map_GetMapInfo(mapID)
...
function AL:MakeList(raids)
	...
	for _, r in T.pairs(raids) do 
		...
		local c = colours[exp]
		local mapName = GetMapName(map_id_list[r])
		if c and mapName then
			list[r] = "|c" .. c .. mapName
		end
	end
	return list
		...
		if mid ~= nil then
			if checkFOD(mid) then
				...
				for k, v in T.pairs (map_id_list) do
					...
					vMap = GetMapInfo(v)
					midMap = GetMapInfo(mid)
					if v == mid then
						rk = k
						break
					elseif vMap and midMap and vMap["name"] == midMap["name"] then
						rk = k
						break
					end
				end
...
function AL:SetLogging(override)
	...
	local isOnGarrisonMap = C_Garrison and C_Garrison.IsOnGarrisonMap and C_Garrison:IsOnGarrisonMap()
	local isDungeon = (instanceType == "party") and not isOnGarrisonMap
	...
	if AL.db.challenge and isDungeon and difficulty == 28 then
		local level = GetActiveKeystoneInfo and GetActiveKeystoneInfo()
		if level and level >= AL.db.mythiclevel then goLog = true end
	end
