...
local currency994 = T.GetCurrencyInfo and T.GetCurrencyInfo(994)
if currency994 then
	IgnoreCurrency[currency994] = true
end
...
local function UpdateMerchantInfo()
	...
	if (_G.MerchantFrame.page > T.math_max(1, T.math_ceil(visibleMerchantItems / _G.MERCHANT_ITEMS_PER_PAGE))) then
		_G.MerchantFrame.page = T.math_max(1, T.math_ceil(visibleMerchantItems / _G.MERCHANT_ITEMS_PER_PAGE))
	end

	-- Show correct page count based on number of items shown
	_G.MerchantPageText:SetFormattedText(_G.MERCHANT_PAGE_NUMBER, _G["MerchantFrame"].page, T.math_ceil(visibleMerchantItems / _G.MERCHANT_ITEMS_PER_PAGE))
		...
		if (index <= visibleMerchantItems) then
			...
			if itemButton and itemButton:IsShown() then
				if not itemButton.text then
					itemButton.text = KUI:CreateText(itemButton, "OVERLAY", 10)
					itemButton.text:SetPoint("BOTTOMRIGHT", 0, 2)
				else
					itemButton.text:SetText("")
				end

				-- ItemLevel on MerchantFrame
...
local function RebuildMerchantFrame()
	...
	if extraCurrencyInset and extraCurrencyBg and moneyInset then
		extraCurrencyInset:ClearAllPoints()
		extraCurrencyInset:SetPoint("BOTTOMRIGHT", moneyInset, "BOTTOMLEFT", 0, 0)
		extraCurrencyInset:SetPoint("TOPLEFT", moneyInset, "TOPLEFT", -165, 0)
		extraCurrencyBg:ClearAllPoints()
		extraCurrencyBg:SetPoint("TOPLEFT", extraCurrencyInset, "TOPLEFT", 3, -2)
		extraCurrencyBg:SetPoint("BOTTOMRIGHT", extraCurrencyInset, "BOTTOMRIGHT", -3, 2)
	end
...
local function MerchantSkinInit()
	...
	if not E.Retail and not E.TBC then return end -- MoP Classic: skip the custom merchant frame rebuild to avoid protected layout mutations
