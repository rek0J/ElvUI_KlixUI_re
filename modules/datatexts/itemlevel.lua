...
local function OnEnter(self)
	T.table_wipe(tooltipOrder)
	T.table_wipe(ArtifactsIlvl)
	local avgItemLevel, avgEquipItemLevel = self.avgItemLevel, self.avgEquipItemLevel
	DT:SetupTooltip(self)
	DT.tooltip:AddDoubleLine(TOTAL, T.math_floor(avgItemLevel), 1, 1, 1, 0, 1, 0)
	DT.tooltip:AddDoubleLine(GMSURVEYRATING3, T.math_floor(avgEquipItemLevel), 1, 1, 1, 0, 1, 0)
	DT.tooltip:AddLine(" ")
...
local function ValueColorUpdate(hex, r, g, b)
	displayString = T.string_join("", "%s:", " ", hex, "%d / %d|r")
	if lastPanel ~= nil then OnEvent(lastPanel) end
end
E["valueColorUpdateFuncs"][ValueColorUpdate] = true

DT:RegisterDatatext("Item Level (KUI)", KUI.Title, {"LOADING_SCREEN_DISABLED", "PLAYER_EQUIPMENT_CHANGED", "UNIT_INVENTORY_CHANGED"}, OnEvent, nil, nil, OnEnter)