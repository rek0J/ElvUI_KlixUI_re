...
local framelist = {
	...
	--["ElvUF_Player"] = {"player", _G["ElvUF_Player"] and _G["ElvUF_Player"]:GetHeight() or 0, "TOPRIGHT", "TOPLEFT", -5, 2, "RIGHT", "LEFT", -3, 0},
}
for i = 1, 5 do
	local arenaFrame = _G["ElvUF_Arena"..i]
	if arenaFrame then
		framelist["ElvUF_Arena"..i] = {"arena"..i, arenaFrame:GetHeight(), T.unpack(frameposition)}
	end
end
...
local function SanitizeDiminishingSpells()
	for spellID in T.pairs(KUI.DiminishingSpells) do
		if not T.GetSpellInfo(spellID) then
			KUI.DiminishingSpells[spellID] = nil
		end
	end
end
