...
local GetAllSelectedPvpTalentIDs = T.C_SpecializationInfo_GetAllSelectedPvpTalentIDs or (C_SpecializationInfo and C_SpecializationInfo.GetAllSelectedPvpTalentIDs)
local LearnPvpTalent = T.LearnPvpTalent or _G.LearnPvpTalent
...
function KBTP:GetTalentInfos()
    ...
    for i = 1, T.GetMaxTalentTier() do
        for j = 1, 3 do
            local talentID, name, texture, selected, available, spellid, tier, column = T.GetTalentInfo(i, j, activeSpecGroup)
            talentInfos[k] = {}
...
function KBTP:GetPvpTalentInfos()
    return GetAllSelectedPvpTalentIDs and GetAllSelectedPvpTalentIDs() or {}
end
...
function KBTP:GetProfile(index)
    ...
    return currentSpec and KUIDataDB[self.playerClass].specs[currentSpec].profiles[index] or nil
end

-- Returns a list of all profiles for the current spec
function KBTP:GetAllProfiles()
    local currentSpec = GetSpecialization and GetSpecialization()
    return currentSpec and KUIDataDB[self.playerClass].specs[currentSpec].profiles or {}
end
...
function KBTP:InsertProfile(profile)
    ...
    T.table_insert(KUIDataDB[self.playerClass].specs[currentSpec].profiles, profile)
end
...
function KBTP:RemoveProfile(index)
    local currentSpec = GetSpecialization and GetSpecialization()
    if not currentSpec then return end
    T.table_remove(KUIDataDB[self.playerClass].specs[currentSpec].profiles, index)
end
...
function KBTP:ActivateProfile(index)
    ...
    if index ~= "new" then
        ...
        if LearnPvpTalent and table.length(profile.pvpTalents) == 4 then
            for i = 1, 4 do
...
function KBTP:BuildFrame()
    ...
    if TalentProfiles_main == nil then
        ...
        mainFrame:Size(_G.PlayerTalentFrame.NineSlice:GetWidth(), _G.PlayerTalentFrame.NineSlice:GetHeight())
        mainFrame:Point("CENTER", _G.PlayerTalentFrame.NineSlice, "CENTER", 0, -20)
    end
    ...
    if TalentProfiles_profilesDropDown == nil then
        ...
        dropdown:Point("TOPLEFT", TalentProfiles_main, "TOPLEFT", 100, -13)
        ...
        TalentProfiles_profilesDropDownButton:Width(TalentProfiles_profilesDropDownButton:GetHeight())
        ...
        TalentProfiles_profilesDropDown:Height(btn_height)
    end
    ...
    if TalentProfiles_btnApply == nil then
        ...
        btnApply:Size(btn_width, btn_height)
        btnApply:SetText("Apply")
        btnApply:Point("TOPLEFT", dropdown, "TOPRIGHT", btn_sepX, -2)
    ...
    if TalentProfiles_btnSave == nil then
        ...
        btnSave:Size(btn_width, btn_height) -- was w100
        btnSave:SetText("Save")
        btnSave:Point("TOPLEFT", btnApply, "TOPRIGHT", btn_sepX, 0)
    ...
    if TalentProfiles_btnRemove == nil then
        ...
        btnRemove:Size(btn_width, btn_height)
        btnRemove:SetText("Remove")
        btnRemove:Point("TOPLEFT", btnSave, "TOPRIGHT", btn_sepX, 0)
    ...
    local enabled = GetSpecialization and KBT.selectedSpec == GetSpecialization()
