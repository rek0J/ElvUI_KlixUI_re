...
function KA:CreateStatCategory(catName, text, noop)
	...
	if not statsPane[catName] then
		local ok, frame = T.pcall(T.CreateFrame, "Frame", nil, statsPane, "CharacterStatFrameCategoryTemplate")
		if not ok or not frame then
			return
		end

		statsPane[catName] = frame
		if frame.Title then
		...
		if frame.backdrop then
			...
			frame.backdrop:SetPoint("CENTER")
			frame.backdrop:SetWidth(150)
			frame.backdrop:SetHeight(18)
		end
	end
	return catName
end

local function DisableStatCategoryDrag(frame)
	if not frame or frame.KuiDragDisabled then return end

	local toolbar = frame.Toolbar or (frame.GetName and frame:GetName() and _G[frame:GetName().."Toolbar"])
	if toolbar then
		if toolbar.SetMovable then
			toolbar:SetMovable(false)
		end
		toolbar:SetScript("OnDragStart", nil)
		toolbar:SetScript("OnDragStop", nil)
	end
	...
	frame.KuiDragDisabled = true
end

function KA:DisableStatCategoryDragging()
	local statsPane = _G.CharacterStatsPane
	if not statsPane then return end

	if PAPERDOLL_STATCATEGORIES then
		for catIndex = 1, #PAPERDOLL_STATCATEGORIES do
			local category = PAPERDOLL_STATCATEGORIES[catIndex]
			if category and category.categoryFrame then
				DisableStatCategoryDrag(statsPane[category.categoryFrame])
			end
		end
	end

	for i = 1, 7 do
		DisableStatCategoryDrag(_G["CharacterStatsPaneCategory"..i])
	end
end

function KA:ResetAllStats()
	local offenseCategory = KA:CreateStatCategory("OffenseCategory", STAT_CATEGORY_ATTACK)
	local defenceCategory = KA:CreateStatCategory("DefenceCategory", DEFENSE)
	...
	if offenseCategory then
		T.table_insert(PAPERDOLL_STATCATEGORIES, {
...
function KA:PaperDollFrame_UpdateStats()
	...
	local attributesCategory = statsPane.AttributesCategory
	local characterItemLevelText = _G["CharacterFrame"] and _G["CharacterFrame"].ItemLevelText

	totalShown = 0
	local total, equipped = T.GetAverageItemLevel()
	if E.db.KlixUI.armory.stats.IlvlFull and itemLevelFrame and itemLevelFrame.Value then
		if characterItemLevelText and characterItemLevelText:IsShown() then
            characterItemLevelText:Hide()
        end
        if not itemLevelFrame.Value:IsShown() then
            itemLevelFrame.Value:Show()
        end
		if E.db.KlixUI.armory.stats.IlvlColor then
			...
			itemLevelFrame.Value:SetFormattedText("%s%.2f|r |cffffffff/|r %s%.2f|r", E:RGBToHex(R, G, B), equipped, E:RGBToHex(avColor.r, avColor.g, avColor.b), total)
		else
			itemLevelFrame.Value:SetFormattedText("%.2f |cffffffff/|r %.2f", equipped, total)
		end
	elseif itemLevelFrame and itemLevelFrame.Value then
		itemLevelFrame.Value:SetTextColor(T.GetItemLevelColor())
		T.PaperDollFrame_SetItemLevel(itemLevelFrame, "player");
	end

	if itemLevelCategory and itemLevelFrame then
		itemLevelCategory:SetPoint("TOP", statsPane, "TOP", 0, 8)
		if attributesCategory then
			attributesCategory:SetPoint("TOP", itemLevelFrame, "BOTTOM", 0, 6)
		end
	end
	...
	if itemLevelCategory and itemLevelFrame then
		itemLevelCategory:Show();
		if itemLevelCategory.Title then
			itemLevelCategory.Title:FontTemplate(E.LSM:Fetch('font', E.db.KlixUI.armory.stats.catFonts.font), E.db.KlixUI.armory.stats.catFonts.size, E.db.KlixUI.armory.stats.catFonts.outline)
		end
		itemLevelFrame:Show();
	elseif itemLevelCategory then
		itemLevelCategory:Hide()
	end
