...
if (faction == FACTION_ALLIANCE or locFaction == FACTION_ALLIANCE) then
    ...
    KBF_BFAWorldBosses[2345] = "Ivus the Decayed" -- Add the horde version as well
    ...
    KBF_BFAWorldBosses[2329] = "Ivus the Forest Lord"
			...
			bonusFilter = {
				...
				args = {
					...
					bfaCategory = {
						...
						args = {
                                                        nyalotha       = KBF:GenerateRaidSettings("Ny'alotha, the Waking City", KBF_Nyalotha, true, true, true, true),
    ...
    if (difficultyID == 8) then
        ...
        elseif (E.db.KlixUI.loot.bonusFilter.disableKeystoneLevelToggle == true and KBF_ShowBonusRoll == false) then
            ...
            if C_ChallengeMode and C_ChallengeMode.GetCompletionInfo then
                local _, l, _, _, _ = C_ChallengeMode.GetCompletionInfo()
                level = l
            end
            if level and (level < T.tonumber(E.db.KlixUI.loot.bonusFilter.disableKeystoneLevel)) then
                KBF:HideRoll()
            end
        end
