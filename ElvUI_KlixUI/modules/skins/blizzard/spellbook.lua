...
local function styleSpellBook()
	...
	if SpellButton_UpdateButton then
		hooksecurefunc("SpellButton_UpdateButton", function(self)
			...
			local slot, slotType = T.SpellBook_GetSpellBookSlot(self)
			local isPassive = T.IsPassiveSpell(slot, SpellBookFrame.bookType)
			...
			if slotType == "FUTURESPELL" then
				local level = T.GetSpellAvailableLevel(slot, SpellBookFrame.bookType)
				if level and level > T.UnitLevel("player") then
	...
	for _, button in T.pairs(professionbuttons) do
	...
	hooksecurefunc("FormatProfession", function(frame, index)
		if index then
			local _, texture = T.GetProfessionInfo(index)
	...
	hooksecurefunc("UpdateProfessionButton", function(self)
		...
		local isPassive = T.IsPassiveSpell(spellIndex, SpellBookFrame.bookType)
		if isPassive then
