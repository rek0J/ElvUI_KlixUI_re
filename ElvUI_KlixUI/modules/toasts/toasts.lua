...
local function RegisterToastEvent(event, method)
	return T.pcall(KT.RegisterEvent, KT, event, method)
end

local function HasGarrisonToastSupport()
	return T.C_Garrison_GetBasicMissionInfo and T.C_Garrison_GetFollowerInfo and T.C_Garrison_IsOnGarrisonMap
		and T.C_Garrison_GetCompleteTalent and T.C_Garrison_GetTalent
...
if C_Timer and C_Timer.NewTicker then
    ...
    -- MoP Classic: C_Timer.NewTicker not available, skip text animation
end
...
local function EnableAchievementToasts()
    if KT.db.achievement_enabled then
        RegisterToastEvent("ACHIEVEMENT_EARNED")
        RegisterToastEvent("CRITERIA_EARNED")
    end
end
...
local function HookArchaeologyProgressBar()
	local progressBar = _G.ArcheologyDigsiteProgressBar
	local anim = progressBar and progressBar.AnimOutAndTriggerToast
	if not anim then return end

	anim:SetScript("OnFinished", ArcheologyProgressBarAnimOut_OnFinished)
	return true
end

local function EnableArchaeologyToasts()
    if not HookArchaeologyProgressBar() and type(_G.ArchaeologyFrame_LoadUI) == "function" then
        local hooked = false

        hooksecurefunc("ArchaeologyFrame_LoadUI", function()
			if not hooked and HookArchaeologyProgressBar() then
				hooked = true
			end
		end)
    end
    ...
    if KT.db.archaeology_enabled then
        RegisterToastEvent("ARTIFACT_DIGSITE_COMPLETE")
    end
end
...
local function EnableGarrisonToasts()
    ...
    if KT.db.garrison_8_0_enabled or KT.db.garrison_7_0_enabled or KT.db.garrison_6_0_enabled then
        RegisterToastEvent("GARRISON_FOLLOWER_ADDED")
        RegisterToastEvent("GARRISON_MISSION_FINISHED")
        RegisterToastEvent("GARRISON_RANDOM_MISSION_ADDED")

        if KT.db.garrison_6_0_enabled then
            RegisterToastEvent("GARRISON_BUILDING_ACTIVATABLE")
        end

        if KT.db.garrison_8_0_enabled or KT.db.garrison_7_0_enabled then
            RegisterToastEvent("GARRISON_TALENT_COMPLETE")
        end
    end
end
...
local function LFGToast_SetUp(isScenario)
    ...
    if isScenario and T.C_Scenario_GetInfo then
...
function KT:LFG_COMPLETION_REWARD()
    if T.C_Scenario_IsInScenario and T.C_Scenario_TreatScenarioAsDungeon and T.C_Scenario_GetInfo
        and T.C_Scenario_IsInScenario() and not T.C_Scenario_TreatScenarioAsDungeon() then
...
local function EnableInstanceToasts()
    if KT.db.instance_enabled then
        RegisterToastEvent("LFG_COMPLETION_REWARD")
    end
end
...
local function EnableSpecialLootToasts()
    if KT.db.loot_special_enabled then
        RegisterToastEvent("LOOT_ITEM_ROLL_WON")
        RegisterToastEvent("SHOW_LOOT_TOAST")
        RegisterToastEvent("SHOW_LOOT_TOAST_LEGENDARY_LOOTED")
        RegisterToastEvent("SHOW_LOOT_TOAST_UPGRADE")
        RegisterToastEvent("SHOW_PVP_FACTION_LOOT_TOAST")
        RegisterToastEvent("SHOW_RATED_PVP_REWARD_TOAST")
...
local function EnableCommonLootToasts()
    if KT.db.loot_common_enabled then
        RegisterToastEvent("CHAT_MSG_LOOT")
    end
end
...
local function EnableCurrencyLootToasts()
    if KT.db.loot_currency_enabled then
        RegisterToastEvent("CHAT_MSG_CURRENCY")
    end
end
...
local function EnableGoldLootToasts()
    if KT.db.loot_gold_enabled then
        ...
        RegisterToastEvent("PLAYER_MONEY")
    end
end
...
local function EnableRecipeToasts()
    if KT.db.recipe_enabled then
        RegisterToastEvent("NEW_RECIPE_LEARNED")
    end
end
...
local function WorldQuestToast_SetUp(questID)
    ...
    local _, _, worldQuestType, rarity, _, tradeskillLineIndex = T.C_QuestLog_GetQuestTagInfo(questID)
...
local function EnableWorldToasts()
    if KT.db.world_enabled and HasWorldToastSupport() then
        RegisterToastEvent("SCENARIO_COMPLETED")
        RegisterToastEvent("QUEST_TURNED_IN")
        RegisterToastEvent("QUEST_LOOT_RECEIVED")
    end
end
...
local function EnableTransmogToasts()
    if KT.db.transmog_enabled and HasTransmogToastSupport() then
        RegisterToastEvent("TRANSMOG_COLLECTION_SOURCE_ADDED")
        RegisterToastEvent("TRANSMOG_COLLECTION_SOURCE_REMOVED")
    end
end
...
function KT:PostAlertMove()
    if not CanManageBlizzardAlertFrame() then return end
...
function KT:Initialize()
    ...
    if CanManageBlizzardAlertFrame() then
